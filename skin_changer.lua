-- Universal cosmetic skin changer, GitHub DB loader, core v0.4.6 first-person overlay isolation candidate.
...
local engine = {
    ...
    overlayOriginalLTM = setmetatable({}, {__mode = "k"}), -- source clone LTM, before context masking
    lastStatus = {},
    warned = {},
...
local function partPath(part, modelRoot)
    local tokens = {}
    local cursor = part
    while cursor ~= modelRoot do
        local parent = cursor.Parent
        if not parent then return nil end
        local ordinal = 0
        for _, sibling in ipairs(parent:GetChildren()) do
            if sibling.Name == cursor.Name and sibling.ClassName == cursor.ClassName then
                ordinal += 1
            end
            if sibling == cursor then break end
        end
        table.insert(tokens, 1, cursor.ClassName .. ":" .. cursor.Name .. ":" .. tostring(ordinal))
        cursor = parent
    end
    return table.concat(tokens, "/")
end

local function indexedBaseParts(modelRoot)
    local map, count = {}, 0
    for _, object in ipairs(modelRoot:GetDescendants()) do
        if object:IsA("BasePart") then
            local key = partPath(object, modelRoot)
            if not key or map[key] then return nil, 0, "ambiguous source part path" end
            map[key] = object
            count += 1
        end
    end
...
function engine:buildVariant(root, record)
    ...
    local originalParts, originalCount, mapError = indexedBaseParts(src)
    ...
    local clonedParts, clonedCount, cloneError = indexedBaseParts(replica)
    if not clonedParts or clonedCount ~= originalCount then
        holder:Destroy()
        return nil, cloneError or "source/clone parts differ"
    end

    for key, original in pairs(originalParts) do
        local clonePart = clonedParts[key]
        if not clonePart or clonePart.ClassName ~= original.ClassName then
            holder:Destroy()
            return nil, "missing or mismatched clone for " .. key
        end

        local overrides = record.sourceMeshOverrides
        local perMesh = original:IsA("MeshPart") and overrides
            and overrides[ident(original.MeshId)] or nil
        ...
        local relative = srcAnchor.CFrame:ToObjectSpace(original.CFrame)
        placeAndWeld(clonePart, liveAnchor, relative)
    end
    holder.Parent = root
    return holder
end
...
function engine:hideOriginal(familyName, root, holder)
    ...
    for _, part in ipairs(root:GetDescendants()) do
        if part:IsA("BasePart") and not part:FindFirstAncestor("__SkinChangerOverlay")
...
-- Only one of the first-person and equipped-third-person COSMETIC clones
-- should render at a time. The game sometimes displays the equipped tool
-- behind its ViewModel, especially during zoom/equip transitions.
-- All changes are local visual transparency; gameplay parts are untouched.
local function setOverlayVisible(engine, entry, visible)
    ...
    for _, part in ipairs(holder:GetDescendants()) do
        if part:IsA("BasePart") then
            local original = engine.overlayOriginalLTM[part]
            if original == nil then
                original = part.LocalTransparencyModifier
                engine.overlayOriginalLTM[part] = original
            end
            local desired = visible and original or 1
...
local function localCameraIsFirstPerson()
    local cam = workspace.CurrentCamera
    local ch = charsRoot()
    local head = ch and ch:FindFirstChild("Head", true)
    if not head then
        local avatar = player.Character
        head = avatar and avatar:FindFirstChild("Head", true)
    end
    if not cam or not head or not head:IsA("BasePart") then return false end
    return (cam.CFrame.Position - head.Position).Magnitude < 1.45
end

function engine:updateOverlayVisibility()
    if not self.running then return end
    for familyName, familyOverlays in pairs(self.overlays) do
        local chosen = self.selections[familyName]
        local record = chosen and db.skins[chosen]
        if record and (record.kind == "variantModel" or record.kind == "modelSwap") then
            local held = familyOverlays.equipped
            local vm = familyOverlays.firstPerson
            -- Only suppress a context if its alternative clone actually exists.
            -- If no ViewModel clone exists, retain the equipped model instead.
            if held and held.holder and held.holder.Parent
                and vm and vm.holder and vm.holder.Parent then
                local firstPerson = localCameraIsFirstPerson()
                setOverlayVisible(self, held, not firstPerson)
                setOverlayVisible(self, vm, firstPerson)
            elseif held and held.holder and held.holder.Parent then
                setOverlayVisible(self, held, true)
            elseif vm and vm.holder and vm.holder.Parent then
                setOverlayVisible(self, vm, true)
            end
        end
    end
end
...
function engine:disable()
    ...
    self.overlays = {}
    self.hidden = {}
    self.overlayOriginalLTM = setmetatable({}, {__mode = "k"})
...
print("[SkinChanger] GitHub core v0.4.6 FP overlay isolation test started, database", db.version,
